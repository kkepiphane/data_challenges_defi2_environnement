...
with g2:
    ...
    encart("constat",
           ...
           f"({fr(dfr['perte_actuelle_ha_an'])} ha/an), sur la <b>santé</b> "
...
LEVIERS = [
    ...
    },
    {
        ...
        "pourquoi": f"Le couvert recule de "
                    f"{fr(dfr['perte_actuelle_ha_an'])} ha par an depuis "
                    f"{dfr['regime_depuis']}, sans une seule année de reprise. "
                    f"Le rythme a été divisé par {rg['ralentissement']:.1f} "
                    f"après {rg['regimes'][0]['fin']} — mais la série nationale, "
                    f"interpolée entre {rg['n_mesures_independantes']} mesures, "
                    f"serait incapable de détecter un retournement récent.",
...
perte_2030 = dfr["perte_actuelle_ha_an"] * (1 - (v_attr / 100) * (v_cuis / 100))
ha_sauves = (dfr["perte_actuelle_ha_an"] - perte_2030) * 5
...
kpi_row([
    ...
    ("Perte forestière en 2030", f"{fr(perte_2030)} ha/an",
     f"contre {fr(dfr['perte_actuelle_ha_an'])} ha/an aujourd'hui", C["risque"]),
...
JAUGES = [
    ...
    ("Réduction de la perte forestière", 0,
     100 * (dfr["perte_actuelle_ha_an"] - perte_2030) / dfr["perte_actuelle_ha_an"], 50, "%",
     C["foret"]),
]
fig2 = go.Figure()
