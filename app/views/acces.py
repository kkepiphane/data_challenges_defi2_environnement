...
with onglet1:
    ...
    st.write("")
    encart("constat",
           f"<b>{ec['valeur']:.0f} points d'écart</b> ({ec['urbain']:.0f} % contre "
           f"{ec['rural']:.0f} %), et ce n'est pas un retard qui se comble : sur "
           f"{er['annee']-er['annee_depart']} ans le rural a gagné "
           f"{er['valeur']-er['valeur_depart']:.0f} points quand l'urbain saturait. "
           f"Atteindre {cible} % en 2030 suppose <b>{fr(pers_an/1000)} 000 "
           f"raccordements ruraux par an</b>, contre {fr(pers_tend_an/1000)} 000 "
           f"aujourd'hui.")
    encart("action",
           "Un tel facteur ne s'obtient pas en prolongeant le réseau national, dont "
           "le coût au raccordement croît avec la distance et la dispersion de "
           "l'habitat. Il s'obtient par des mini-réseaux solaires et des kits "
           "domestiques, déployables sans attendre la ligne moyenne tension.")
...
with onglet2:
    ...
    encart("alerte",
           f"<b>Les coupures se raréfient, mais elles se généralisent.</b> La "
           f"fréquence baisse ({fr(fi['coupures_mois_ref'], 1)} → "
           f"{fr(fi['coupures_mois'], 1)} par mois) pendant que la part d'entreprises "
           f"touchées passe de {fi['part_entreprises_ref']:.0f} % à "
           f"<b>{fi['part_entreprises']:.0f} %</b>. Le réseau s'est étendu plus vite "
           f"qu'il ne s'est consolidé.")
    encart("action",
           "Pour un village éloigné, la question n'est donc pas « réseau ou "
           "solaire ? » mais « service fiable ou service intermittent ? ». Un "
           "mini-réseau solaire avec stockage offre une disponibilité prévisible et "
           "immédiate, là où le réseau apporterait plus tard un courant instable.")
