...
# --- tapis roulant démographique : recalculé ici sans réutiliser le pipeline
er_s = wb["EG.ELC.ACCS.RU.ZS"]
ans_tr = sorted(set(pr) & set(er_s))
t0, t1 = ans_tr[0], ans_tr[-1]
s0 = pr[t0] * (1 - er_s[t0] / 100)
s1 = pr[t1] * (1 - er_s[t1] / 100)
TR = R["tapis_roulant"]
...
verifie(f"Ruraux sans électricité en {t1}", s1, TR["sans_elec_fin"], tol=1,
        unite="pers.")
verifie("Variation du stock", s1 - s0, TR["variation"], tol=1, unite="pers.")
verifie("Effet démographie", (pr[t1] - pr[t0]) * (1 - er_s[t0] / 100),
        TR["effet_demographie"], tol=1, unite="pers.")
verifie("Effet électrification", -pr[t1] * (er_s[t1] - er_s[t0]) / 100,
        TR["effet_acces"], tol=1, unite="pers.")
verifie("Seuil de stagnation (croissance rurale)", pr[t1] - pr[t1 - 1],
        TR["seuil_stagnation"], tol=1, unite="/an")
# L'identité qui porte la conclusion : le déficit annuel de raccordement,
# multiplié par le nombre d'années, doit redonner la hausse du stock.
d0 = TR["annee_debut_decennie"]
n_ans = t1 - d0
racc = sum(pr[a] * er_s[a] / 100 - pr[a - 1] * er_s[a - 1] / 100
           for a in range(d0 + 1, t1 + 1)) / n_ans
seuil = sum(pr[a] - pr[a - 1] for a in range(d0 + 1, t1 + 1)) / n_ans
verifie(f"Raccordements moyens {d0}-{t1}", racc,
        TR["raccordements_moyens_10ans"], tol=1, unite="/an")
verifie(f"Seuil moyen {d0}-{t1}", seuil, TR["seuil_moyen_10ans"], tol=1,
        unite="/an")
verifie(f"Variation du stock {d0}-{t1} (= déficit × {n_ans})",
        (seuil - racc) * n_ans, TR["variation_decennie"], tol=1, unite="pers.")
...
# --- les deux régimes de la série forestière, recomptés à la main.
# Arrondi au dixième de km² : les dernières décimales de la source sont un
# artefact d'interpolation, pas une mesure.
